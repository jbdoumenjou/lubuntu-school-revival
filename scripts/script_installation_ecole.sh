...
# =============================================================================
# SCRIPT D'INSTALLATION COMPLÈTE - ÉCOLE PRIMAIRE
# Compatible : Celeron 570, 1Go RAM, chipset SiS 771/671
# Système : Lubuntu 18.04.6 LTS 32-bit
# =============================================================================
...
# Création du fichier de configuration graphique
sudo mkdir -p /etc/X11/xorg.conf.d/
...
# Désactiver les services de veille
sudo systemctl mask sleep.target suspend.target hibernate.target hybrid-sleep.target
...
# Configuration DNS Cloudflare anti-pub
sudo tee /etc/systemd/resolved.conf > /dev/null <<EOF
[Resolve]
DNS=1.1.1.2#cloudflare-dns.com
DNS=1.0.0.2#cloudflare-dns.com
...
# Applications éducatives principales
sudo apt install -y gcompris-qt tuxmath tuxtype

echo "✅ Applications éducatives principales installées"
...
# LibreOffice avec support français
...
sudo apt install -y stellarium marble
...
# Multimédia
sudo apt install -y vlc ubuntu-restricted-extras audacity
...
# Installation Firefox ESR (Extended Support Release)
sudo apt install -y firefox-esr

# Configuration navigateur par défaut
sudo update-alternatives --install /usr/bin/x-www-browser x-www-browser /usr/bin/firefox-esr 200

echo "✅ Firefox ESR installé et configuré"
...
sudo apt install -y kanagram childsplay
...
# Police spécialisée dyslexie
sudo apt install -y fonts-opendyslexic

# Synthèse vocale française
sudo apt install -y espeak espeak-data-fr festival festvox-fr-mbrola-fr1
...
# Installation uniquement via APT (pas de snap)
sudo apt install -y scratch
...
# =============================================================================
# 18. RÉSUMÉ ET INSTRUCTIONS FINALES
# =============================================================================
